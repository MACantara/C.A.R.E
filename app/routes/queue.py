...
def api_update_status():
    ...
    try:
        ...
        # Update status
        queue_entry.status = QueueStatus(new_status)
        if delay_reason:
            queue_entry.delay_reason = delay_reason
        if notes:
        ...
        current_app.logger.error(f"Queue update error: {e}")
        return jsonify({"error": "Failed to update queue status"}), 500
